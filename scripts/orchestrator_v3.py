...
KyberSlash-aware data collection.

Key changes from v2:
- Targets the FO transform implicit rejection path (valid vs mutated CT)
- Records message HW (proxy for division operands in compression)
- Higher trace count per key for robust KDE estimation
- Separate handling of valid vs invalid ciphertexts

Output: data/raw_timing_traces_v3.csv
"""
...
KEYGEN_BIN = os.path.join(PROJECT_DIR, "src", "keygen_helper_v3")
TIMING_BIN = os.path.join(PROJECT_DIR, "src", "timing_harness_v2")
OUTPUT_CSV = os.path.join(PROJECT_DIR, "data", "raw_timing_traces_v3.csv")
...
def generate_samples(num_samples, mutation_rate=0.5):
    """Run keygen_helper_v3 and parse output."""
    proc = subprocess.run(
        [KEYGEN_BIN, str(num_samples), str(mutation_rate)],
        capture_output=True, text=True, timeout=3600
    )
    if proc.returncode != 0:
        print(f"keygen_helper_v3 failed: {proc.stderr}", file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="KyberSlash-aware orchestrator")
    parser.add_argument("--num-keys", type=int, default=5000,
                        help="Number of distinct keys (default: 5000)")
    parser.add_argument("--num-repeats", type=int, default=100,
                        help="Decap repeats per key (default: 100)")
    parser.add_argument("--mutation-rate", type=float, default=0.5,
                        help="Fraction of invalid ciphertexts (default: 0.5)")
    parser.add_argument("--load-threads", type=int, default=4)
    parser.add_argument("--output", type=str, default=OUTPUT_CSV)
    args = parser.parse_args()

    total = args.num_keys * args.num_repeats
    print(f"[Orchestrator v3] KyberSlash-aware collection")
    print(f"  {args.num_keys} keys x {args.num_repeats} repeats = {total:,} traces")
    print(f"  Mutation rate: {args.mutation_rate} (fraction of invalid CTs)")

    print(f"\n[Step 1] Generating {args.num_keys} samples...")
