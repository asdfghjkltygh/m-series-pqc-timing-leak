...
We release sca-triage, an open-source tool that triages TVLA false positives using pairwise secret-group decomposition and permutation-validated mutual information, and propose a two-stage evaluation protocol that prevents compliant implementations from being wrongly failed. Organizations on CNSA 2.0 timelines cannot afford a compliance bottleneck built on a broken test.
