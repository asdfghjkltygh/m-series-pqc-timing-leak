...
Prior work (Standaert et al., Whitnall & Oswald) documented the detection-exploitability gap in theory on evaluation boards, and dudect (Reparaz et al., 2017) applies Welch's t-test to detect non-constant-time behavior in software implementations. We go further: we systematically quantified the ISO 17825 failure rate specifically for the NIST ML-KEM standard under FIPS 140-3 testing conditions on shipping production hardware, and provide a structured triage workflow when the protocol fails. Cross-platform replication (Apple ARM + Intel x86) with different root causes but identical TVLA failure modes is a systemic finding, not a platform curiosity. The positive control shows TVLA is broken in BOTH directions: false positives on hardened code AND the KyberSlash vulnerability (d = 0.094) falls below TVLA's detection floor while ML classification catches it. The FIPS 140-3 compliance angle transforms this from a research curiosity into a deployment-blocking problem with real-world consequences.
...
Prior work (Standaert et al., Whitnall & Oswald) documented the detection-exploitability gap in theory on evaluation boards, and dudect (Reparaz et al., 2017) tests individual implementations for constant-time violations. We go beyond both: we measured the gap at scale on shipping production hardware running the NIST-standardized PQC algorithm, and provide a triage protocol when ISO 17825 fails. Cross-platform replication (Apple ARM + Intel x86) with different root causes but identical TVLA failure modes is a systemic finding, not a platform curiosity. The positive control shows TVLA is broken in BOTH directions: false positives on hardened code AND the KyberSlash vulnerability (d = 0.094) falls below TVLA's detection floor while ML classification catches it. The FIPS 140-3 compliance angle transforms this from a research curiosity into a deployment-blocking problem with real-world consequences.
