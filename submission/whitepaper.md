...
### Apple Silicon: DMP Synchronization (Empirically Confirmed Architectural)
...
We empirically confirmed that the Apple Silicon confound is architectural by running the symmetric control experiment. The symmetric harness pre-generates all 50,000 random (ciphertext, secret key) pairs into memory arrays before measurement begins; both fixed and random modes execute identical code paths during the timed loop (array index → decaps → record timing). No keygen or encaps occurs inside the measurement loop in either mode.

The result is unambiguous and striking. The symmetric harness **still fails TVLA catastrophically**: |t| = 62.49 with a variance ratio of 7.71x (fixed over random). This is far worse than the original asymmetric harness result of |t| = 8.42. In fact, the asymmetric harness — when run as a control in the same experiment — **passes TVLA** (|t| = 3.00, below the 4.5 threshold) because the keygen+encaps cache pollution in random mode was actually *masking* the DMP effect by adding noise that brought the two distributions closer together.

The DMP synchronization on repeated fixed data is the dominant, primary confound on Apple Silicon. With a clean measurement loop, the DMP effect is 7.7x larger in the variance signature and produces a t-statistic 7.4x higher than the asymmetric harness. The fixed group shows a mean of 594.5 cycles with std of 208.2 (bimodal: fast mode with rare catastrophic DMP mispredictions), while the random group shows a mean of 532.6 with std of 75.0 (unimodal: DMP never converges).

We note that we did not use Performance Monitoring Counters (PMCs) to directly observe DMP state transitions; our attribution is based on the consistency of the observed variance signature with the known DMP behavioral model and the symmetric control experiment confirming the effect is data-dependent, not code-path-dependent.
...
We have not yet run the symmetric control experiment on Intel (it requires x86 hardware). The candidates for the Intel signal are: (a) keygen+encaps cache pollution (harness asymmetry), (b) intrinsic cache replacement behavior on varying vs repeated data (architectural), and (c) OS scheduler effects (Linux CFS). The Apple Silicon symmetric control showed that eliminating harness asymmetry made the DMP effect *stronger*, not weaker — but Intel lacks the DMP, so the result may differ. Running the symmetric harness on Intel is immediate future work.
...
This cross-platform replication elevates the finding from a platform-specific curiosity to a systemic problem. On Apple, the symmetric control experiment proves the confound is architectural — the DMP effect is 7.4x stronger when harness asymmetry is removed. On Intel, the relative contribution of harness asymmetry vs architectural effects remains to be isolated. In both cases, the practical implication is the same: TVLA produces false positives on production hardware, and pairwise decomposition is needed to distinguish real leakage from artifacts.
...
**Harness symmetry.** Our original TVLA harnesses execute `keygen()` + `encaps()` before each random-mode measurement but not before fixed-mode measurements. We ran the symmetric control experiment on Apple Silicon: a harness that pre-generates all random (ciphertext, secret key) pairs into memory arrays before measurement, ensuring identical code paths. The symmetric harness fails TVLA even more severely (|t| = 62.49 vs 8.42), confirming the Apple confound is architectural. On Intel, the symmetric control has not been run; the relative contribution of harness asymmetry vs architectural cache effects remains to be isolated.
