...
To validate that sca-triage adds genuine diagnostic capability, we ran a head-to-head comparison against dudect (Reparaz et al., DATE 2017), the most widely used academic tool for timing leakage detection. Both were given the same Apple Silicon asymmetric harness data:

| Tool | Patched v0.15.0 (no real leak) | Vulnerable v0.9.0 (KyberSlash) |
|------|------|------|
| **dudect** | |t| = 8.42 → NON-CONSTANT-TIME | |t| = 1.04 → Underpowered |
| **TVLA (ISO 17825)** | |t| = 8.42 → FAIL | |t| = 1.04 → Underpowered |
| **sca-triage** | FALSE_POSITIVE (pairwise d=0.0003, MI=0.0 bits) | REAL_LEAKAGE (XGBoost 56.6% vs 52.8% chance) |

dudect and TVLA use the same Welch's t-test and reach identical conclusions: both flag the patched (non-leaking) implementation as failing, and both are underpowered on the vulnerable implementation at 25K traces. sca-triage correctly classifies both: the patched failure is a false positive (pairwise decomposition finds zero secret dependence), and the vulnerable version contains real leakage (ML classifier detects exploitable timing dependence despite the underpowered t-test). The temporal-drift confound affects any tool that relies solely on fixed-vs-random comparison — including dudect.
